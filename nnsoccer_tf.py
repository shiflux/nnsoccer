...
class SoccerPredictorTF:
    def __init__(self):
        ...
        a = tf.set_random_seed(1234)
        self.tf_session = tf.Session(a)
        ...
        for r in rounds:
            ...
            for match in temp_data:
                ...
                if not (homet in features and awayt in features):
                    continue
                lx = features[homet] + features[awayt] + [var1-var2 for var1 in features[homet] for var2 in features[awayt] if features[homet].index(var1) == features[awayt].index(var2)]
                print(len(lx))
                ly = temp_data[match]
                if ly == "":
                    break
    ...
    def getFeatures(self, serie, season, my_features=settings.my_global_features):
        ...
        #return self.get_extra_features(serie=serie, season=season, dict=temp_dict) # works only for serie a
        return temp_dict
