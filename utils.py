...
def fit(X, y, X1, y1, C1=2, gamma=0.2):
    ...
    for x in range(len(y1)):
        ...
        if predicted[x] == 1:
            print ("aaaaaaaaaaaaaaaaaaaa", predicted[x] == y1[x], predicted_prob[x])
            if max_threshold > flag1 >= threshold and max_threshold > flag1 >= threshold: #and numpy.absolute(predicted_prob[x][0] - predicted_prob[x][2]) < (1 - threshold)/2:
                if predicted[x] == y1[x]:
                    temp_res.append(1)
                else:
                    temp_res.append(0)
