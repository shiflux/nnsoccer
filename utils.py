...
def fit(X, y, X1, y1, C1=2, gamma=0.2):
    ...
    for x in range(len(y1)):
        ...
        if numpy.absolute(predicted_prob[x][predicted[x]] - predicted_prob[x][(predicted[x]+1)%3]) > threshold and numpy.absolute(predicted_prob[x][predicted[x]]- predicted_prob[x][(predicted[x]+2)%3]) > threshold:
            if predicted[x] == y1[x]:
                temp_res.append(1)
            else:
                temp_res.append(0)
